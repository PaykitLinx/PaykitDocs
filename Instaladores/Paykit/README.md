...
1. **Hotfix**               - Correção de crash intermitente na transação de QRCode.
1. **Feature**              - Inclusão de botões para copiar o código Cópia e Cola e imprimir o QRCode via interface gráfica do Paykit.
1. **Feature**              - Melhoria de performance na transação de QRCode, deixando mais rápido o fluxo de upload da imagem para o Pinpad e diminuindo o tempo de espera entre o acionamento da transação e o QRCode aparecer em tela para o operador de caixa

##  Versão: 8.22.24.0019
    Descrição: Melhoria no tratamento de erros em transações QR.
    Plataforma: Windows x32, Ubuntu 22.04 x64, Ubuntu 20.04 x64, Ubuntu 20.04 x32, CentOS 7.5 x32, Oracle Linux 9 x32 e Rocky Linux 8.9 x32
    Data: 10/06/2026

1. **Hotfix**               - Retentativa de envio da Transacao de QRCode em caso de falha na comunicação com o servidor TEF.

##  Versão: 8.22.24.0018
    Descrição: Correção de cancelamento da transação QR.
    Plataforma: Windows x32, Ubuntu 22.04 x64, Ubuntu 20.04 x64, Ubuntu 20.04 x32, CentOS 7.5 x32, Oracle Linux 9 x32 e Rocky Linux 8.9 x32
    Data: 18/05/2026

1. **Hotfix**               - Impedindo cancelamento de transação de QR após receber mensagem de transação aprovada do TEF.

##  Versão: 8.22.24.0017
    Descrição: Correções no GP
    Plataforma: Windows x32, Ubuntu 22.04 x64, Ubuntu 20.04 x64, Ubuntu 20.04 x32, CentOS 7.5 x32, Oracle Linux 9 x32 e Rocky Linux 8.9 x32
    Data: 06/05/2026

1. ** Hotfix**               - Ignorar linhas inválidas no arquivo intpos.001.
1. ** Bugfix**               - Correções na comunicação com o pinpad para evitar crashs intermitentes em transações de cartão.
...
1. **Feature**              - Em caso de transacao de cancelamento para transação de QRCode na rede QRLinx, será impedida uma atualização de tabelas.
...
1. **Hotfix**               - Correção de fluxo para cartões Amex em fluxos de falha de comunicação de rede para evitar crash.
1. **Hotfix**               - Corrigindo fluxo ObtemLogTransacaoJson para não gerar crashs em caso de acentuação.
1. **Hotfix**               - Correção método AtivaPDV que estava manipulando dados da automação comercial indevidamente
...
1. **Bugfix**               - FrotaJson correção no fluxo para transacoes frota via Cielo
1. **Bugfix**               - Correção de crash ao transacionar usando Ubuntu 22.04 e conexão sem SSL.
...
1. **Bugfix**               - FrotaJson corrigindo coleta de senha do portador do cartão
...
1. **Bugfix**               - Correção de erro no pinpad quando realiza uma reimpressão de cupom
...
1. **Hotfix**             - Correção na lista de transações exibidas na tela do GP (Integração troca de arquivos)
...
1. **Hotfix**             - FrotaJson correções de fluxo, para que a transação funcione de forma adequada.
...
1. **Bugfix**             - Correção para evitar o envio de dados inválidos para o pinpad no comando GOX para a adquirente Redecard.
...
1. **Bugfix**             - Corrigindo leitura de dados do cartão
...
1. **Bugfix**             - Corrigindo leitura de dados do cartão
1. **Bugfix**             - Atualização do arquivo GPDirecao (Troca de arquivos) com correções de problemas. 
...
1. **Bugfix**             - Tratando novos AID's de voucher da Elo para Elo PAT.
1. **Bugfix**             - Tratamento de aids exclusivos para o PAT ELO
1. **Bugfix**             - Ajustando fluxo da transação de débito completa com saque.
1. **Feature**            - Implementação SingleTap Pagseguro
1. **Feature**            - Adicionando transação troco saque para a rede pagseguro (Transação de débito completa).
...
1. **Feature**            - Transação QRCode para especificação da PagSeguro (adicionado fallback no débito)
1. **Feature**            - Adicionando transação de frota json para a adquirente Cielo
1. **Feature**            - Implementação da transação FrotaJSON
1. **Feature**            - Implementação da adquirente Mercado Pago
1. **Feature**            - Implementação da adquirente Picpay
1. **Feature**            - Alteração para habilitar transações CTLS para a PICPAY e demais redes Genericas.
1. **Feature**            - Atualizando rede generica para criptografar dados
1. **Bugfix**             - Ajuste nas operações de pontos da Rede Generica
1. **Feature**            - Adicionando forma de pagamento QRCode Elo
